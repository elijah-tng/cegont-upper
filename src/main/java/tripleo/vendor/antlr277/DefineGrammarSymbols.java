...
public class DefineGrammarSymbols implements ANTLRGrammarParseBehavior {
        ...
        if (literal != null) {
            ...
            if (sl != null) {
                ...
                if (name == null || sl.getLabel() != null) {
                    tool.warning(
                            "Redefinition of literal in tokens {...}: " + literal,
                            grammar.getFilename(),
                            tokliteral.getLine(),
                            tokliteral.getColumn());
                    return;
                } else if (name != null) {
            ...
            if (name != null) {
                ...
                if (ts != null) {
                    ...
                    if (ts instanceof StringLiteralSymbol) {
                        tool.warning(
                                "Redefinition of token in tokens {...}: " + name,
                                grammar.getFilename(),
                                tokliteral.getLine(),
                                tokliteral.getColumn());
                        return;
    ...
    public void endOptions() {
        ...
        if (grammar.exportVocab == null && grammar.importVocab == null) {
            ...
            if (tokenManagers.containsKey(DEFAULT_TOKENMANAGER_NAME)) {
                ...
                // System.out.println("No tokenVocabulary for '" + grammar.getClassName() + "', using default '" +
                // tm.getName() + "'");
                grammar.setTokenManager(tm);
                return;
            }
            // no shared vocab for file, make new one
            // System.out.println("No exportVocab for '" + grammar.getClassName() + "', creating default '" +
            // grammar.exportVocab + "'");
        ...
        if (grammar.exportVocab == null && grammar.importVocab != null) {
            ...
            ImportVocabTokenManager tm = new ImportVocabTokenManager(
                    grammar,
        ...
        if (grammar.exportVocab != null && grammar.importVocab != null) {
            ...
            if (grammar.importVocab.equals(grammar.exportVocab)) {
                tool.error(
                        "exportVocab of " + grammar.exportVocab + " same as importVocab; probably not what you want");
            }
    ...
    public void refHeaderAction(Token name, Token act) {
        ...
        if (name == null) key = "";
        else key = StringUtils.stripFrontBack(name.getText(), "\"", "\"");

        // FIXME: depending on the mode the inserted header actions should
        // be checked for sanity.
        if (headerActions.containsKey(key)) {
            if (key.equals("")) tool.error(act.getLine() + ": header action already defined");
            else tool.error(act.getLine() + ": header action '" + key + "' already defined");
        }
    ...
    public void refToken(
            Token assignId, Token t, Token label, Token args, boolean inverted, int autoGenType, boolean lastInRule) {
        _refToken(assignId, t, label, args, inverted, autoGenType, lastInRule);
    }

    /** Reference a token */
    public void _refToken(
            Token assignId, Token t, Token label, Token args, boolean inverted, int autoGenType, boolean lastInRule) {
        String id = t.getText();
    ...
    public void setFileOption(Token key, Token value, String filename) {
        if (key.getText().equals("language")) {
            if (value.getType() == ANTLRParser.STRING_LITERAL) {
                ...
                tool.error(
                        "language option must be string or identifier", filename, value.getLine(), value.getColumn());
            }
            ...
            if (!language.equals("Cpp")) {
                ...
                if (key.getText().equals("noConstructors")) {
                    ...
                    if (value.getType() != ANTLRParser.STRING_LITERAL) {
                        tool.error(
                                key.getText() + " option must be a string",
                                filename,
                                value.getLine(),
                                value.getColumn());
                    } else {
            ...
            if (!language.equals("Cpp") && !language.equals("CSharp")) {
                tool.error(
                        key.getText() + " option only valid for C++ and C# (a.k.a CSharp)",
                        filename,
                        key.getLine(),
                        key.getColumn());
            } else {
                if (value.getType() != ANTLRParser.STRING_LITERAL) {
                    tool.error(
                            key.getText() + " option must be a string", filename, value.getLine(), value.getColumn());
                } else {
    ...
    public void setGrammarOption(Token key, Token value) {
        if (key.getText().equals("tokdef") || key.getText().equals("tokenVocabulary")) {
            ...
            if (value.getType() == ANTLRParser.RULE_REF || value.getType() == ANTLRParser.TOKEN_REF) {
                ...
                tool.error(
                        "exportVocab must be an identifier", grammar.getFilename(), value.getLine(), value.getColumn());
            }
            ...
            if (value.getType() == ANTLRParser.RULE_REF || value.getType() == ANTLRParser.TOKEN_REF) {
                ...
                tool.error(
                        "importVocab must be an identifier", grammar.getFilename(), value.getLine(), value.getColumn());
            }
        } else if (key.getText().equals("k")) {
            if (grammar instanceof TreeWalkerGrammar && !value.getText().equals("1")) {
