...
public class LLkAnalyzer implements LLkGrammarAnalyzer {
            ...
            for (int j = i + 1; j < nalts; j++) {
                ...
                if (haveAmbiguity) {
                    ...
                    /* If greedy=true and *one* empty alt shut off warning. */
                            ...
                            && blk.greedy
                            && ((ai.head instanceof BlockEndElement && !(aj.head instanceof BlockEndElement))
                                    || (aj.head instanceof BlockEndElement && !(ai.head instanceof BlockEndElement)))) {
    ...
    public Lookahead FOLLOW(int k, RuleEndElement end) {
        ...
        if (end.cache[k] != null) {
            ...
            if (re.cache[k] == null) {
                ...
                if (DEBUG_ANALYZER) {
                    System.out.println("combining FOLLOW(" + k + ") for " + rule + ": from "
                            + end.cache[k].toString(",", charFormatter, grammar) + " with FOLLOW for "
                ...
                if (DEBUG_ANALYZER) {
                    System.out.println("saving FOLLOW(" + k + ") for " + rule + ": from "
                            + end.cache[k].toString(",", charFormatter, grammar));
                }
    ...
    public Lookahead look(int k, RuleRefElement rr) {
        ...
        if (p.cycle != null) {
            tool.error(
                    "infinite recursion to rule " + p.cycle + " from rule " + rr.enclosingRuleName,
                    grammar.getFilename(),
                    rr.getLine(),
                    rr.getColumn());
        }
