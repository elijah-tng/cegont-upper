        ...
        for (int i = 0; i < lexRules.size(); i++) {
            ...
            if (!r.isDefined()) {
                ...
                if (r.access.equals("public")) {
                    ...
                    RuleRefElement rr = new RuleRefElement(
                            g, new CommonToken(ANTLRTokenTypes.RULE_REF, r.getId()), GrammarElement.AUTO_GEN_NONE);
...
public void noAutoGenSubRule() {
    ...
    public void defineRuleName(Token r, String access, boolean ruleAutoGen, String docComment)
        ...
        if (r.type == ANTLRTokenTypes.TOKEN_REF) {
            ...
            if (grammar instanceof LexerGrammar) {
                tool.error(
                        "Lexical rule names must be upper case, '" + r.getText() + "' is not",
                        grammar.getFilename(),
                        r.getLine(),
                        r.getColumn());
            ...
            if (!analyzer.subruleCanBeInverted(block, grammar instanceof LexerGrammar)) {
                ...
                tool.error(
                        "This subrule cannot be inverted.  Only subrules of the form:" + newline
                                + "    (T1|T2|T3...) or"
                                + newline + "    ('c1'|'c2'|'c3'...)"
                                + newline + "may be inverted (ranges are also allowed).",
                        grammar.getFilename(),
                        block.getLine(),
                        block.getColumn());
            }
        ...
        if (label != null) {
            ...
            for (int i = 0; i < ruleBlock.labeledElements.size(); i++) {
                ...
                if (l != null && l.equals(label.getText())) {
                    tool.error(
                            "Label '" + label.getText() + "' has already been defined",
                            grammar.getFilename(),
                            label.getLine(),
                            label.getColumn());
                    return;
    ...
    public void refTokensSpecElementOption(Token tok, Token option, Token value) {
        ...
        */
        TokenSymbol ts = (TokenSymbol) grammar.tokenManager.getTokenSymbol(tok.getText());
        if (ts == null) {
        ...
        if (option.getText().equals("AST")) {
            ...
            grammar.antlrTool.error(
                    "invalid tokens {...} element option:" + option.getText(),
                    grammar.getFilename(),
                    option.getLine(),
                    option.getColumn());
        }
    ...
    public void refElementOption(Token option, Token value) {
        ...
        */
        ...
        if (e instanceof StringLiteralElement || e instanceof TokenRefElement || e instanceof WildcardElement) {
            ...
            tool.error(
                    "cannot use element option (" + option.getText() + ") for this kind of element",
                    grammar.getFilename(),
                    option.getLine(),
                    option.getColumn());
        }
    ...
    public void refRule(Token idAssign, Token r, Token label, Token args, int autoGenType) {
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
                tool.error(
                        "AST specification ^ not allowed in lexer", grammar.getFilename(), r.getLine(), r.getColumn());
            }
        }
    ...
    public void refToken(
            Token idAssign, Token t, Token label, Token args, boolean inverted, int autoGenType, boolean lastInRule) {
        if (grammar instanceof LexerGrammar) {
            // In lexer, token references are really rule references
            if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
                tool.error(
                        "AST specification ^ not allowed in lexer", grammar.getFilename(), t.getLine(), t.getColumn());
            }
            if (inverted) {
    ...
    public void synPred() {
        if (context().block.not) {
            tool.error(
                    "'~' cannot be applied to syntactic predicate",
                    grammar.getFilename(),
                    context().block.getLine(),
                    context().block.getColumn());
        }
