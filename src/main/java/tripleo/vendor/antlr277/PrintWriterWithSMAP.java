...
public class PrintWriterWithSMAP extends PrintWriter {
    ...
    public void startMapping(int sourceLine) {
        mapLines = true;
        if (sourceLine != JavaCodeGenerator.CONTINUE_LAST_MAPPING) currentSourceLine = sourceLine;
    }
    ...
    protected void mapLine(boolean incrementOutputLineCount) {
        if (mapLines && anythingWrittenSinceMapping) {
            ...
            if (!outputLines.contains(outputLine)) outputLines.add(outputLine);
        }
        if (incrementOutputLineCount) currentOutputLine++;
        if (!mapSingleSourceLine) currentSourceLine++;
    ...
    public void checkChar(int c) {
        ...
        else if (!Character.isWhitespace((char) c)) anythingWrittenSinceMapping = true;
